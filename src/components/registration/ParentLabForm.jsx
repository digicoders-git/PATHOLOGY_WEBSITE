...
const ModernDropdown = ({
  ...
  const displayValue = options.find((opt) =>
    typeof opt === "object" ? opt[idKey] === value : opt === value,
  );
  const labelToShow = displayValue
    ...
    <div className="space-y-1.5">
      ...
      {type === "select" ? (
        ...
        >
          ...
          <span
            className={`font-bold text-[10px] uppercase tracking-wider transition-colors duration-300 ${value ? "text-primary" : "text-black"}`}
          >
            {label}
          </span>
        </div>
...
const ParentLabForm = () => {
    ...
    const initData = async () => {
      try {
        ...
        if (parentsRes.success)
          setParentOptions(parentsRes.data.filter((p) => p.status));
      } catch (error) {
  ...
  const validate = () => {
    let newErrors = {};
    if (!formData.parent) newErrors.parent = "Required";
